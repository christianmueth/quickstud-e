  ...
  return (
    <div className="mx-auto flex max-w-5xl flex-col gap-6 p-6">
      <section className="border-b border-slate-200 pb-6">
        <h1 className="text-3xl font-semibold tracking-tight text-slate-950">Plan</h1>
        {checkoutState === "success" ? (
          <div className="mt-4 rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-900">
            Checkout complete. Access updates shortly.
          </div>
        ) : null}
      ...
      <section className="grid gap-6 lg:grid-cols-[1fr_1fr]">
        <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          <h2 className="text-2xl font-semibold text-slate-950">{formatPlanLabel(billing.plan)}</h2>
          <div className="mt-5 space-y-3 text-sm text-slate-700">
            <p>
              Status: <span className="font-medium text-slate-950">{billing.subscriptionStatus}</span>
            </p>
            <p>
              This month: <span className="font-medium text-slate-950">{billing.monthlyGenerationCount}</span>
              {billing.monthlyGenerationLimit ? ` of ${billing.monthlyGenerationLimit}` : " (unlimited)"}
            </p>
            <p>
              Remaining: <span className="font-medium text-slate-950">{billing.monthlyGenerationsRemaining ?? "Unlimited"}</span>
            </p>
            <p>
              Renews: <span className="font-medium text-slate-950">{nextBillingDate || "Not scheduled"}</span>
            </p>
          </div>
        ...
        <div className="rounded-3xl border border-amber-200 bg-amber-50 p-6 shadow-sm">
          <h2 className="text-2xl font-semibold tracking-tight text-slate-950">Premium</h2>
          <p className="mt-2 text-sm text-slate-700">Unlimited study tools. $2.99/month.</p>
        </div>
      </section>
